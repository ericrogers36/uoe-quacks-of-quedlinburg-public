...
def simulate_game_vps_wrapper(policybundle):
    return simulate_game(policybundle).VPs

def simulate_multiple_parallel(policybundle, n):
    with ProcessPoolExecutor() as ex:
        return np.array(list(ex.map(lambda _: simulate_game_vps_wrapper(policybundle), range(n))))
